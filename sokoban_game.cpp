...
bool condOver(float p[][2],float b1[][2],float b2[][2],float b3[][2]);
bool condPairs(float x[][2],float y[][2]);

int t;
float xX = 0,yY = 0,prevxX = 0,prevyY = 0;
int box_num = 0;
...
bool condOver(float p[][2],float b1[][2],float b2[][2],float b3[][2])
{
	if(condPairs(p,b1) || condPairs(p,b2) || condPairs(p,b3) || condPairs(b1,b2) || condPairs(b1,b3) || condPairs(b2,b3) )
		return false;
	else
		return true;
}
...
/*	if(condOver(p,b1,b2,b3)==true)
	{
		motion(p,prevxX,prevyY);
		if(box_num==1);
			motion(b1,prevxX,prevyY);
		else if(box_num == 2)
			motion(b2,prevxX,prevyY);
		else if(box_num == 3)
			motion(b3,prevxX,prevyY);
	}
*/
	/*
	   cout << b1[1][0]<<" "<<p[0][0]<<endl;
	   cout << b1[1][1]<<" "<<p[0][1]<<endl;
	   cout << b1[2][0]<<" "<<p[3][0]<<endl;
	   cout << b1[2][1]<<" "<<p[3][1]<<endl;
	 */
